...
func _ready() -> void:
	if not OS.is_debug_build():
		set_process(false)
		return
	for file_name in [REQUEST_FILE, RESPONSE_FILE, REQUEST_FILE + ".tmp", RESPONSE_FILE + ".tmp"]:
		var path := _user_path(file_name)
...
func _dispatch_async(req: Dictionary) -> Dictionary:
	...
	match action:
		...
		"execute_script":
			...
			if code.begins_with("call_node|"):
				return _execute_node_call(code)
			var expr := Expression.new()
			if expr.parse(code) != OK:
...
func _execute_node_call(code: String) -> Dictionary:
	var parts := code.split("|")
	if parts.size() < 3:
		return {"error": "call_node requires path and method"}
	var node := _resolve_node(parts[1])
	if node == null:
		return {"error": "node not found: %s" % parts[1]}
	var method_name := StringName(parts[2])
	if not node.has_method(method_name):
		return {"error": "method not found: %s" % parts[2]}
	var args: Array = []
	for index in range(3, parts.size()):
		args.append(_parse(parts[index]))
	var val = node.callv(method_name, args)
	return {"result": str(val)}
...
func _resolve_node(path: String) -> Node:
	...
	if path_text.begins_with("/root/") or path_text.begins_with("root/"):
		return get_node_or_null(NodePath(path_text if path_text.begins_with("/") else "/%s" % path_text))
	if path_text.begins_with("/"):
		return get_node_or_null(NodePath(path_text))
	var scene := get_tree().current_scene
	if scene == null:
		return null
...
func _write_response(data: Dictionary) -> void:
	var response_path := _user_path(RESPONSE_FILE)
	var temp_path := response_path + ".tmp"
	...
	if file:
		...
		DirAccess.rename_absolute(temp_path, response_path)
